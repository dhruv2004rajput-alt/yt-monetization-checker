...
// Groq models to try in order (fallback if one fails)
const MODELS = [
  ...
  return await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `Analyze this YouTube channel for monetization eligibility: ${channelUrl}`
        }
      ],
      temperature: 0.7,
      max_tokens: 4000
  ...
  for (const model of MODELS) {
    try {
      ...
      if (!response.ok) {
        ...
        // If rate limited (429) or model not available, try next model
        if (errorCode === 429 || errorCode === 404) {
          console.log(`Model ${model} failed with code ${errorCode}, trying next model...`);
          lastError = errData?.error?.message || "Model failed";
          continue;
        }
      ...
      try {
        ...
        console.error(`JSON parse failed for model ${model}. Raw text:`, text.substring(0, 200));
